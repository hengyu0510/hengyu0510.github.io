...
function bindGallery() {
  ...
  const configuredPhotos = (galleryContainer.dataset.galleryImages || '')
    .split(',')
    .map((src) => src.trim())
    .filter(Boolean);

  const photos = configuredPhotos.map((src, index) => ({
    src,
...
async function init() {
  ...
  if (hero) {
    window.__HENGYU_HERO__ = hero;
    window.__HENGYU_HERO_CONFIG__ = heroConfig;
  }
  ...
  function frame(now) {
    ...
    state.scroll = hero
      ? lerp(state.scroll, state.scrollTarget, state.reducedMotion ? 0.14 : 0.08)
      : state.scrollTarget;
